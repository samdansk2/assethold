...
ticker_list = fc.fdata.get_tickers_dow()


def set_up_app(choices=ticker_list):
...
def index():
    current_app.logger.debug("function name: {}; http request :{}".format(
        'index', request.method))
...
def get_form_submitted_value():
    ...
    current_app.logger.debug(
        "Form Submitted ticker :{}".format(form_submitted_value))
