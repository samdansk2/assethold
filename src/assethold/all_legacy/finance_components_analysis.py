...
class FinanceAnalysis():
    ...
    def run_long_term_analysis(self):
        ...
        self.company_info.update(
            {'breakout_trend_indicators': breakout_trend_json})
        insider_info = self.evaluate_insider_trend(self.insider_df)
        self.company_info.update({'insider_info': insider_info})
        if self.option_data:
            call_effective_value_dict = self.call_analysis(
                self.option_data, self.current_price)
            no_of_closes_call_records = 10
            self.filter_call_analysis_data(no_of_closes_call_records,
                                           self.current_price)
            self.company_info.update({
                'optionanalysis': {
                    'call_analysis': call_effective_value_dict
                }
            })
    ...
    def evaluate_insider_trend(self, insider_df):
        ...
        insider_analysis_by_relation_dict = self.get_insider_analysis_by_relation(
            insider_df.copy())
    ...
    def get_insider_summary(self, insider_df):
        if len(insider_df) > 0:
            insider_df = transform_df_datetime_to_str(insider_df,
                                                      date_format='%Y-%m-%d')
    ...
    def get_tootip_for_insider_summary_row(self, row):
        Tooltip = "trasactionType: {}, Insider: {}, Relationship: {}".format(
            row['Transaction'], row['Insider Trading'], row['Relationship'])
        return Tooltip
    ...
    def get_insider_analysis_by_relation(self, insider_df):
        ...
        try:
            columns = [
                'Relationship', 'Share Holding Ratio', 'Actions',
                'Average Cost', 'Tooltip'
            ]
            ...
            for trader_index in range(0, len(inside_traders)):
                ...
                if start_shares > 0:
                    share_holding_ratio = (end_shares /
                                           start_shares).__round__(2)
                else:
                    share_holding_ratio = 1.01

                try:
                    average_cost = (df_insider['#Shares'] * df_insider['Cost']
                                   ).sum() / df_insider['#Shares'].sum()
                ...
                relationship = df_insider['Relationship'].iloc[
                    0] + '_' + trader_intials

                actions = "; ".join(transaction_list)
                data_tooltip = "{0}, Share End/Start # of {1}; End Shares of {2}; Average Cost: {4}, Actions: {3}".format(
                    df_insider['Relationship'].iloc[0], share_holding_ratio,
                    end_shares, actions, average_cost)
                row_array = [
                    relationship, share_holding_ratio, actions, average_cost,
                    data_tooltip
                ]
                self.insider_analysis_by_relation_df.loc[len(
                    self.insider_analysis_by_relation_df)] = row_array
            insider_analysis_by_relation_dict = self.insider_analysis_by_relation_df.to_dict(
                orient='records')
    ...
    def get_insider_analysis_by_timeline(self, insider_df):
        ...
        try:
            columns = [
                'tradeDate', 'Actions', 'Share Holding Ratio', 'Average Cost',
                'Tooltip'
            ]
            ...
            for trade_date_index in range(0, len(trade_dates)):
                ...
                for transaction_type in unique_transactions:
                    ...
                    try:
                        average_cost = (
                            df_transaction['#Shares'] * df_transaction['Cost']
                        ).sum() / df_transaction['#Shares'].sum()
                        average_cost = round(average_cost, 3)
                    except:
                        average_cost = None

                    data_tooltip = ''
                    data_tooltip = data_tooltip + 'Average Cost: {}; '.format(
                        average_cost)
                    transaction_tooltip = ''
                    for transaction_index in range(0, len(df_transaction)):
                        inside_trader = df_transaction['Insider Trading'].iloc[
                            transaction_index]
                        trader_intials = get_initials_from_name(inside_trader)
                        relationship = df_trade_date['Relationship'].iloc[
                            0] + '_' + trader_intials
                        transaction_tooltip = transaction_tooltip + relationship + "; "
                    data_tooltip = data_tooltip + transaction_tooltip
                    row_array = [
                        trade_date, transaction_type, share_holding_ratio,
                        average_cost, data_tooltip
                    ]
    ...
    def call_analysis(self, option_data, current_price):
        ...
        call_analysis_dict = self.call_effective_value_df.to_dict(
            orient='records')
    ...
    def add_all_ta_features(self, df):
        ...
        if self.cfg['source'] == 'yfinance':
            try:
                self.ta = add_all_ta_features(df,
                                              open="Open",
                                              high="High",
                                              low="Low",
                                              close="Close",
    ...
    def evaluate_call_data(self, option_date, option_data, current_price):
        ...
        for od_index in range(0, len(option_data)):
            ...
            if contract_cost > 0:
                effectiveValuePerShare = contract_cost + (current_price -
                                                          strike)
                ...
                effectiveValuePerShare = 0
            call_analysis_array = [
                option_date, strike, effectiveValuePerShare, tool_tip
            ]
            self.call_effective_value_df.loc[len(
                self.call_effective_value_df)] = call_analysis_array
    ...
    def get_return_on_investment(self):
        ...
        for stock_index in range(0, len(self.stock_data_array)):
            ...
            if self.cfg.stocks[stock_index].__contains__(
                    'purchase_date'
            ) and self.cfg.stocks[stock_index]['purchase_date'] is not None:
            ...
            if self.cfg.stocks[stock_index].__contains__(
                    'initial_investment') and self.cfg.stocks[stock_index][
                        'initial_investment'] is not None:
                initial_investment = self.cfg.stocks[stock_index][
                    'initial_investment']
            else:
                initial_investment = 1000
            if self.cfg.stocks[stock_index].__contains__(
                    'yearly_payment'
            ) and self.cfg.stocks[stock_index]['yearly_payment'] is not None:
                yearly_payment = self.cfg.stocks[stock_index]['yearly_payment']
            else:
                yearly_payment = 1000

            df_temp = df[df.date >= purchase_date]
            df_for_returns = pd.concat([
                df_temp.head(1), df_temp[df_temp.divCash > 0].copy(),
                df.tail(1)
            ],
                                       ignore_index=True,
                                       copy=True)
            ...
            if self.cfg.stocks[stock_index].__contains__(
                    'purchase_price'
            ) and self.cfg.stocks[stock_index]['purchase_price'] is not None:
                purchase_price = self.cfg.stocks[stock_index]['purchase_price']
            else:
                purchase_price = statistics.mean([
                    df_for_returns.loc[0, 'close'], df_for_returns.loc[0,
                                                                       'open']
                ])
            ...
            for row_index in range(0, len(df_for_returns)):
                purchase_price = statistics.mean([
                    df_for_returns.loc[row_index, 'close'],
                    df_for_returns.loc[row_index, 'open']
                ])
                if row_index == 0:
                    df_for_returns.loc[
                        row_index,
                        'no_of_shares'] = no_of_shares_purchased + df_for_returns.loc[
    ...
    def filter_call_analysis_data(self, n, current_price):
        ...
        strike_prices.sort()
        closest_value, closest_value_index = getClosestIntegerInList(
