...
from assethold.common.data import transform_df_datetime_to_str, transform_df_None_to_NULL
from assethold.finance_components_analysis import FinanceAnalysis
from assethold.finance_components_get_data import FinanceGetData
...
class FinanceComponents():
    ...
    def perform_analysis(self, data_dict):
        if 'database' not in self.cfg or not self.cfg['database']['data_exists']:
    ...
    def get_data_exist_status_in_db(self):
        ...
        if self.dbe_connection_status:
            try:
                db_latest_data_exist_status = self.get_data_status_from_db(
                    cfg=None)
            except:
                logging.error(
                    "Error in database data status. Detailed error {0}".format(
                        traceback.format_exc()))
    ...
    def get_data_status_from_db(self, cfg=None):
        ...
        analysis_threshold_datetime = datetime.datetime.utcnow(
        ) + datetime.timedelta(days=-2)
        if df is not None and len(df) == 1 and (
                df.updated_time.iloc[0] is not None) and (
                    df.updated_time.iloc[0] > analysis_threshold_datetime):
            return True
        else:
            return False
    ...
    def get_data_for_UI(self, ticker=None):
        ...
        try:
            ...
            data_dict['updated_time'] = data_dict['updated_time'].strftime(
                '%m-%d-%Y')
        except:
            print(
                "Error getting data from database for ticker {}".format(ticker))
    ...
    def get_stock_analysis_plot_cfg(self, data_dict):
        ...
        insider_by_timeline = insider_info.get(
            'insider_by_timeline', None) if insider_info is not None else None
        insider_summary = insider_info.get(
            'insider_summary', None) if insider_info is not None else None
        insider_df_buy = insider_summary.get(
            'insider_df_buy', None) if insider_summary is not None else None
        insider_df_sell = insider_summary.get(
            'insider_df_sell', None) if insider_summary is not None else None
    ...
    def get_result_df_for_db(self):
        columns = [
            'ticker', 'updated_time', 'sector', 'summary', 'daily_data',
            'breakout_trends', 'insider_info', 'option_analysis', 'status', 'ta'
        ]
        result_df = pd.DataFrame(columns=columns)

        ticker = self.cfg['stocks'][0]['ticker']
        updated_time = datetime.datetime.utcnow().replace(second=0,
                                                          microsecond=0)
        ...
        df_row = [
            ticker, updated_time, sector, summary, daily_data, breakout_trends,
            insider_info, option_analysis, status, ta
        ]
    ...
    def get_daily_data_for_save_to_db(self):
        ...
        daily_data_raw = transform_df_datetime_to_str(daily_data_raw,
                                                      date_format="%Y-%m-%d")
    ...
    def save_analysis_column_to_db(self, cfg_save):
        ...
        try:
            df = self.dbe.executeScriptsFromFile(
                filename, arg_array=[column, data, status, ticker])
        except:
            logging.error(
                "Could not get stock ticker data using query: {}".format(
                    filename))
        ...
        for item in item_list:
            try:
                df = self.dbe.executeScriptsFromFile(
                    ...
                    arg_array=[
                        item, item,
                        json.dumps(sec_ticker_data.get(item),
                                   ensure_ascii=False)
                    ])
            except:
                filename_json = os.path.join(script_working_dir, 'services',
                ...
                logging.error(
                    "Could not update Label : {0}. Saved data to file system. Detailed error {1}"
    ...
    def save_institution_data_sec(self, result):
        ...
        try:
            df = self.dbe.executeScriptsFromFile(filename,
                                                 arg_array=[
                                                     cik, institution, summary,
                                                     reportCalendarOrQuarter,
                                                     signatureDate, infoTable
                                                 ])
        except:
            logging.error(
                "Could not save institution data using query: {}".format(
                    filename))
    ...
    def save_keys(self):
        ...
        filename_json = os.path.join(script_working_dir, 'services',
                                     'StockAnalysis', 'static', 'StockAnalysis',
        ...
        company_tickers_exchange_df = pd.DataFrame(
            company_tickers_exchange['data'],
            columns=company_tickers_exchange['fields'])

        filename = os.path.join(script_working_dir, 'data',
                                self.dbe.server_type, 'sql',
                                'stocks.save_keys.sql')
        ...
        for df_row in range(0, len(company_tickers_exchange_df)):
            ...
            if ticker not in dtf_keys_tickers:
                ...
                logging.debug(
                    f"Keys updated for ticker {ticker}, cik: {cik}".format({
                        'ticker': ticker,
                        'cik': cik
                    }))
    ...
    def get_institution_data_from_db(self, ticker):
        ...
        if ticker is not None:
            try:
                ...
                print("Error getting data from database for ticker {}".format(
                    ticker))
    ...
    def save_insider_data_to_db(self, cfg_save):
        ...
        try:
            ...
            logging.error(
                "Could not save data using query: {}".format(filename))
